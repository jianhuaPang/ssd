...
def new_get_map(MINOVERLAP, draw_plot, score_threhold=0.5, path='./map_out',outpath ='./map_out', totalcount = 0):
    GT_PATH = os.path.join(path, 'ground-truth')
    DR_PATH = os.path.join(path, 'detection-results')
    IMG_PATH = os.path.join(path, 'images-optional')
    TEMP_FILES_PATH = os.path.join(path, '.temp_files')
    RESULTS_FILES_PATH = os.path.join(outpath, 'results')
    ...
    with open(RESULTS_FILES_PATH + "/results.txt", 'w') as results_file:
        ...
        for class_index, class_name in enumerate(gt_classes):
            ...
            infor = '' + str(totalcount) + '\t'
            sum_AP += ap
            text = "{0:.2f}%".format(
                ap * 100) + " = " + class_name + " AP "  # class_name + " AP = {0:.2f}%".format(ap*100)
            infor += "{0:.2f}%".format(ap * 100)+'\t'
            if len(prec) > 0:
                F1_text = "{0:.2f}".format(F1[score_threhold_idx]) + " = " + class_name + " F1 "
                Recall_text = "{0:.2f}%".format(rec[score_threhold_idx] * 100) + " = " + class_name + " Recall "
                Precision_text = "{0:.2f}%".format(prec[score_threhold_idx] * 100) + " = " + class_name + " Precision "
            ...
            if len(prec) > 0:
                infor +=str(score_threhold) + '\t' + "{0:.2f}".format(F1[score_threhold_idx]) + '\t' + "{0:.2f}".format(
                    rec[score_threhold_idx] * 100) + '\t' + "{0:.2f}".format(prec[score_threhold_idx] * 100) + '\t'
                ...
                infor += str(score_threhold) + '\t' + '0.00\t0.00\t0.00%'
            ...
            #     axes.set_xlim([0.0, 1.0])
            #     axes.set_ylim([0.0, 1.05])
            #     fig.savefig(RESULTS_FILES_PATH + "/AP/" + class_name+str(totalcount) + ".png")
            #     plt.cla()
            #
            #     plt.plot(score, F1, "-", color='orangered')
            #     plt.title('class: ' + F1_text + "\nscore_threhold=" + str(score_threhold))
            ...
            #     axes.set_xlim([0.0, 1.0])
            #     axes.set_ylim([0.0, 1.05])
            #     fig.savefig(RESULTS_FILES_PATH + "/F1/" + class_name+str(totalcount) + ".png")
            #     plt.cla()
            #
            #     plt.plot(score, rec, "-H", color='gold')
            #     plt.title('class: ' + Recall_text + "\nscore_threhold=" + str(score_threhold))
            ...
            #     axes.set_xlim([0.0, 1.0])
            #     axes.set_ylim([0.0, 1.05])
            #     fig.savefig(RESULTS_FILES_PATH + "/Recall/" + class_name+str(totalcount) + ".png")
            #     plt.cla()
            #
            #     plt.plot(score, prec, "-s", color='palevioletred')
            #     plt.title('class: ' + Precision_text + "\nscore_threhold=" + str(score_threhold))
            ...
            #     axes.set_xlim([0.0, 1.0])
            #     axes.set_ylim([0.0, 1.05])
            #     fig.savefig(RESULTS_FILES_PATH + "/Precision/" + class_name+str(totalcount) + ".png")
            #     plt.cla()
        ...
        results_file.write("\n# mAP of all classes\n")
        mAP = sum_AP / n_classes
        mAPtext = "mAP = {0:.2f}%".format(mAP * 100)
        results_file.write(mAPtext + "\n")
        print(mAPtext)
    ...
    with open(RESULTS_FILES_PATH + "/results.txt", 'a') as results_file:
        ...
        for class_name in sorted(dr_classes):
            ...
            text += ", fp:" + str(n_det - count_true_positives[class_name]) + ")\n"
            results_file.write(text)
            infor += str(n_det)+'\t' + str(count_true_positives[class_name]) + "\t"+ str(n_det - count_true_positives[class_name]) + "\t"
    ...
    # """
    # Plot the total number of occurences of each class in the "detection-results" folder  本来就是注释掉的
    # """
    # if draw_plot:
    ...
    #         ""
    #     )
    infor += "{0:.2f}%".format(mAP * 100)+'\t'
    # 记录
    with open(os.path.join(outpath, "epoch_data.txt"), 'a') as f:
        f.write(str(infor))
        f.write("\n")
    return mAP
